...
- [~] **🖼 网图节点** —— 各地标网图已下载到 `photos/<country>/<region>/<landmark>_{1,2,3}.jpg`（文件夹/文件名统一 lowercase snake_case；每地标 3 张候选，本地未提交）。✅ **已全部接进地图数据** `data/footprints.json`（13 国、所有州/省 + 地标共 **330 个节点**，坐标据实填 3 位；与 `photos/` 文件夹一一对应，仅跳过与本人照片节点重复的少数）。**剩**：确认版权（可商用 / CC）+ 把图真正显示进故事卡。
